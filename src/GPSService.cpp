...
void GPSService::read_GPGSV(const NMEASentence& nmea){
	...
	*/
	...
	try
	{
		...
		int entriesInPage = (int)(nmea.parameters.size() - 3) >> 2;	//first 3 are not satellite info
...
void GPSService::read_GNGGA(const NMEASentence& nmea){
	/* -- EXAMPLE --
	$GNGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*47

	$GNGGA,205630.945,3346.1070,N,08423.6687,W,0,03,,30.8,M,-30.8,M,,0000*73		// ATLANTA!!!!

	$GNGGA,113645.20,5026.01587,N,03038.27373,E,1,07,2.02,137.8,M,25.5,M,,*4C
	...
	*/
	...
	{
		NMEAParseError pe("GPS Number Bad Format [$GNGGA] :: " + ex.message, nmea);
		throw pe;
	}
	catch (NMEAParseError& ex)
	{
		NMEAParseError pe("GPS Data Bad Format [$GNGGA] :: " + ex.message, nmea);
		throw pe;
	}
}
...
void GPSService::read_GNRMC(const NMEASentence& nmea){
	/*  -- EXAMPLE ---
	$GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W*6A
	$GPRMC,235957.025,V,,,,,,,070810,,,N*4B
	$GPRMC,061425.000,A,3346.2243,N,08423.4706,W,0.45,18.77,060914,,,A*47
	...
	*/
	...
	{
		NMEAParseError pe("GPS Number Bad Format [$GNRMC] :: " + ex.message, nmea);
		throw pe;
	}
	catch (NMEAParseError& ex)
	{
		NMEAParseError pe("GPS Data Bad Format [$GNRMC] :: " + ex.message, nmea);
		throw pe;
	}
}
...
void GPSService::read_GNGSA(const NMEASentence& nmea){
	...
	$GNGSA,A,3,18,21,22,14,27,19,,,,,,,4.4,2.7,3.4*32
	...
	*/
	...
	{
		NMEAParseError pe("GPS Number Bad Format [$GNGSA] :: " + ex.message, nmea);
		throw pe;
	}
	catch (NMEAParseError& ex)
	{
		NMEAParseError pe("GPS Data Bad Format [$GNGSA] :: " + ex.message, nmea);
		throw pe;
	}
}
